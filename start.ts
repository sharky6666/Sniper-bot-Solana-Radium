...
export async function processRaydiumPool(id: PublicKey, poolState: LiquidityStateV4) {
  ...
  if (id.toString().endsWith('pump')) {
    logger.info(`Skipping pool with id ending in 'pump': ${id.toString()}`);
    return;
  }

  const balance = await solanaConnection.getTokenAccountBalance(quoteTokenAssociatedAddress, commitment);
  if (quoteToken.symbol === 'WSOL' && balance.value.uiAmount !== null && balance.value.uiAmount < Number(QUOTE_AMOUNT)) {
    logger.info(`Insufficient WSOL balance: ${balance.value.uiAmount} ${quoteToken.symbol}`);
    return;
  }

  if (poolState.status.toNumber() !== 6) {
    logger.info(`Liquidity pool isn't locked: ${id.toString()}`);
    return;
  }

  const mintInfo = await solanaConnection.getParsedAccountInfo(poolState.baseMint);
  if (mintInfo.value && 'parsed' in mintInfo.value.data) {
    const mintData = mintInfo.value.data.parsed.info;
    if (mintData.mintAuthority !== null) {
      logger.info(`Mint authority is not renounced for token: ${poolState.baseMint.toString()}`);
      return;
    }
    if (mintData.isMutable) {
      logger.info(`Token is mutable: ${poolState.baseMint.toString()}`);
      return;
    }
  }
